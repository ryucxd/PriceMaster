...
{
    public partial class frmTraditionalLossReason : Form
    {
        public int quote_id { get; set; }
        public frmTraditionalLossReason(int _quote_id)
        {
            InitializeComponent();
            quote_id = _quote_id;
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string note = "";
            sql = "SELECT custom_feedback FROM [order_database].dbo.quotation_feed_back WHERE quote_id = " + quote_id;
            using (SqlConnection conn = new SqlConnection(CONNECT.ConnectionString))
            {
                ...
                sql = "UPDATE [order_database].dbo.quotation_feed_back SET custom_feedback =  '" + note + "' WHERE quote_id = " + quote_id;
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                    cmd.ExecuteNonQuery();

...
{
    public partial class frmTraditionalChaseHistoryNew : Form
    {
        public int chase_id { get; set; }
        public int _quote_id { get; set; }
        public frmTraditionalChaseHistoryNew(int quote_id)
        {
            InitializeComponent();
            _quote_id = quote_id;
            string sql = "select l.id,l.chase_date as [Chase Date],u.forename + ' ' + u.surname as [Full Name],CASE WHEN chase_complete = 0 THEN CAST(0 AS BIT) WHEN chase_complete IS NULL THEN CAST(0 AS BIT) ELSE CAST(1 AS BIT) END AS [Complete] from [order_database].dbo.quotation_chase_log   l " +
        ...
        private void fillChase(int chase_id)
        {
            ...
            string sql = "select chase_date,chase_description,next_chase_date, dont_chase,phone,email,chase_complete from [order_database].dbo.quotation_chase_log  where id = " + chase_id.ToString();
        ...
        {
            //mark chase as comp
            string sql = "UPDATE [order_database].dbo.quotation_chase_log  SET chase_complete = -1 WHERE id = " + chase_id;
            using (SqlConnection conn = new SqlConnection(CONNECT.ConnectionString))
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand(sql, conn))
